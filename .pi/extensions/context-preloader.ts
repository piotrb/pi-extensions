...
function flattenNodes(nodes: RefNode[]): Array<{ path: string; content: string }> {
...
function renderTree(nodes: RefNode[], cwd: string, prefix = ""): string[] {
	...
	for (let i = 0; i < nodes.length; i++) {
		...
		const suffix = node.skipped === "already-loaded"
			? " (already loaded)"
...
export default function (pi: ExtensionAPI): void {
	let cachedSections: Array<{ path: string; content: string }> | null = null;
	...
	pi.on("resources_discover", async (event, ctx) => {
		...
		const roots: Array<{ label: string; children: RefNode[] }> = [];
