...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    parameters: Type.Object({
      // ── core ──────────────────────────────────────────────────────────────
      pattern: Type.String({
        description: "Search pattern. Interpreted as a regex unless fixedStrings is true.",
      }),
      paths: Type.Optional(
        Type.Array(Type.String(), {
          description: "Files or directories to search (default: current working directory).",
        }),
      ),
      ...
      fixedStrings: Type.Optional(
        Type.Boolean({
          description: "Treat pattern as a literal string, not a regex. Equivalent to rg -F.",
        }),
      ),
      ...
      smartCase: Type.Optional(
        Type.Boolean({
          description: "Case-insensitive if pattern is all lowercase, case-sensitive otherwise. Equivalent to rg -S.",
        }),
      ),
      wordRegexp: Type.Optional(
        Type.Boolean({
          description: "Only match whole words (pattern surrounded by word boundaries). Equivalent to rg -w.",
        }),
      ),
      ...
      multiline: Type.Optional(
        Type.Boolean({
          description: "Allow patterns to match across multiple lines. Equivalent to rg -U.",
        }),
      ),
      ...
      typeNot: Type.Optional(
        Type.Union([Type.String(), Type.Array(Type.String())], {
          description: "Exclude files matching a known type. Equivalent to rg -T.",
        }),
      ),
      hidden: Type.Optional(
        Type.Boolean({
          description: "Search hidden files and directories (dotfiles). Equivalent to rg --hidden.",
        }),
      ),
      noIgnore: Type.Optional(
        Type.Boolean({
          description: "Don't respect .gitignore / .ignore / .rgignore files. Equivalent to rg --no-ignore.",
        }),
      ),
      ...
      maxDepth: Type.Optional(
        Type.Integer({
          description: "Limit directory traversal depth. Equivalent to rg --max-depth.",
          minimum: 0,
        }),
      ),
      ...
      context: Type.Optional(
        Type.Integer({
          description: "Show N lines before and after each match. Equivalent to rg -C.",
          minimum: 0,
        }),
      ),
      ...
      count: Type.Optional(
        Type.Boolean({
          description: "Print only the count of matching lines per file. Equivalent to rg -c.",
        }),
      ),
      filesWithMatches: Type.Optional(
        Type.Boolean({
          description: "Print only the names of files with at least one match. Equivalent to rg -l.",
        }),
      ),
      noLineNumber: Type.Optional(
        Type.Boolean({
          description: "Suppress line numbers from output (line numbers are shown by default). Equivalent to rg -N.",
        }),
      ),
      maxCount: Type.Optional(
        Type.Integer({
          description: "Stop reading a file after N matches. Equivalent to rg -m.",
          minimum: 1,
        }),
      ),
      sortBy: Type.Optional(
        Type.Union(
          ...
          {
            description: 'Sort results. "path" is alphabetical. "none" (default) is fastest. Equivalent to rg --sort.',
          },
        ),
      ),
    }),
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      // file filtering
      const globs = params.glob ? (Array.isArray(params.glob) ? params.glob : [params.glob]) : []
      for (const g of globs) args.push("--glob", g)

      const types = params.type ? (Array.isArray(params.type) ? params.type : [params.type]) : []
      for (const t of types) args.push("--type", t)

      const typeNots = params.typeNot ? (Array.isArray(params.typeNot) ? params.typeNot : [params.typeNot]) : []
      ...
      if (params.beforeContext != null) args.push("--before-context", String(params.beforeContext))
      if (params.afterContext != null) args.push("--after-context", String(params.afterContext))
      ...
      if (params.sortBy && params.sortBy !== "none") args.push("--sort", params.sortBy)
      ...
      if (spawnError) {
        return {
          ...
          details: { pattern: params.pattern, paths: params.paths ?? [], totalLines: 0, exitCode: -1 },
          isError: true,
        }
      }
    ...
    renderCall(args, theme) {
      ...
      const globs = args.glob ? (Array.isArray(args.glob) ? args.glob : [args.glob]) : []
      for (const g of globs) parts.push(`--glob='${g}'`)

      const types = args.type ? (Array.isArray(args.type) ? args.type : [args.type]) : []
      for (const t of types) parts.push(`-t${t}`)

      const typeNots = args.typeNot ? (Array.isArray(args.typeNot) ? args.typeNot : [args.typeNot]) : []
      ...
      const patternStr = args.pattern.includes(" ") ? `'${args.pattern}'` : args.pattern
...
function renderLine(line: string, theme: Theme): string {
  ...
  const m = /^([^:\n]+)([:−\-])(\d+)([:−\-])(.*)$/.exec(line)
  if (m) {
    const [, file, sep1, lineNo, sep2, content] = m as unknown as [string, string, string, string, string, string]
